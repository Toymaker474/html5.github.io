...
async function loadNative(){
  const [manifest,...chunks]=await Promise.all([
    fetch('weather.wasm.manifest.json',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('manifest '+r.status);return r.json();}),
    ...PARTS.map(p=>fetch(p,{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error(p+' '+r.status);return r.text();}))
  ]);
  const bytes=decodeBase64(chunks.map(s=>s.trim()).join(''));
  if(bytes.byteLength!==manifest.compiled_bytes)throw new Error(`WASM byte mismatch ${bytes.byteLength}/${manifest.compiled_bytes}`);
  const digest=hex(await crypto.subtle.digest('SHA-256',bytes));
  if(digest!==manifest.sha256||digest!==EXPECTED_SHA)throw new Error('WASM SHA-256 mismatch — refusing simulation boot');
  const mod=await WebAssembly.compile(bytes);
  const imports=WebAssembly.Module.imports(mod);if(imports.length)throw new Error('unexpected WASM imports');
  const instance=await WebAssembly.instantiate(mod,{});const e=instance.exports;
...
function draw(force=false){
  if(!api||(!force&&performance.now()-lastDraw<80))return;lastDraw=performance.now();
  const d=img.data;let k=0;
  for(let z=0;z<SIM_H;z++)for(let x=0;x<SIM_W;x++){
    const elevation=api.genesis_weather_elevation_at(x,z);
    ...
    if(layer!=='hydrology'){
      ...
      if(tornado>0){const q=clamp(.45+tornado/300,0,1);r=mix(r,255,q);g=mix(g,128,q*.65);b=mix(b,48,q*.35);}
    }
    d[k++]=r|0;d[k++]=g|0;d[k++]=b|0;d[k++]=255;
  }
  sx.putImageData(img,0,0);
  const rect=canvas.getBoundingClientRect();const ratio=Math.min(devicePixelRatio||1,2);const w=Math.max(320,Math.round(rect.width*ratio)),h=Math.round(w*SIM_H/SIM_W);
  ...
  // Tornado presentation marker: location/strength come only from native tornado cells.
  if(layer!=='hydrology'){
    const scaleX=canvas.width/SIM_W,scaleY=canvas.height/SIM_H;ctx.lineWidth=Math.max(1.5,scaleX*.55);
    for(let z=0;z<SIM_H;z+=1)for(let x=0;x<SIM_W;x+=1){const t=api.genesis_weather_tornado_at(x,z);if(!t)continue;const px=(x+.5)*scaleX,py=(z+.5)*scaleY,rad=scaleX*(1.2+t/90);ctx.strokeStyle=`rgba(255,221,167,${clamp(.22+t/350,.25,.8)})`;ctx.beginPath();ctx.arc(px,py,rad,0,Math.PI*1.55);ctx.stroke();}
  }
}
...
function frame(t){
  if(booted&&running&&t-lastStep>95){lastStep=t;api.genesis_weather_step(speed);updateMetrics();}
...
(async()=>{
  try{
    $('boot').textContent='VERIFYING COMPILER-PROVEN WASM…';api=await loadNative();booted=true;$('boot').textContent='NATIVE C++ → VERIFIED WASM';$('boot').classList.add('ok');setScenario('storm');requestAnimationFrame(frame);
  }catch(err){console.error(err);$('boot').textContent='FAIL-CLOSED: '+err.message;$('boot').classList.add('bad');running=false;}
})();
