...
export class World {
  ...
  stepCorpse(c,dt){
    ...
    for(let it=0;it<4;it++){for(let i=0;i<4;i++)this.constrain(c.nodes[i],c.nodes[i+1],7,.72);for(const leg of c.legs){const a=c.nodes[leg.anchor];this.constrain(a,leg.knee,10,.62);this.constrain(leg.knee,leg.foot,10,.62);}for(const f of c.feelers){const head=c.nodes[c.dir>0?4:0];this.constrainFollower(head,f.tip,12.5,.55);}for(const n of c.nodes)this.collidePoint(n);for(const leg of c.legs){this.collidePoint(leg.knee);this.collidePoint(leg.foot);}for(const f of c.feelers)this.collidePoint(f.tip);}
