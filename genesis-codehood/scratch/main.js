...
worker.onmessage=e=>{const m=e.data;if(m.type!=='telemetry')return;info.innerHTML=`<b>${m.alive}</b> alive <span>·</span> ${m.corpses||0} corpses <span>·</span> flow ${m.flow.toFixed(2)} <span>·</span> rain ${Math.round(m.rain*100)}%`;const s=m.selected;document.getElementById('selected').textContent=s?(s.alive?`Crawler #${s.id} · ${String(s.intent).toUpperCase()} · energy ${Math.round(s.energy*100)}% · water ${Math.round(s.hydration*100)}% · fatigue ${Math.round(s.fatigue*100)}% · gut ${Math.round(s.stomach*100)}%`:`Crawler #${s.id} · CORPSE · ${s.deathCause||'unknown cause'}`):'Tap an organism';updateAudio(m);};
