...
const worker=new Worker('./scratch/environment-worker.js',{type:'module'});
...
worker.onmessage=e=>{const m=e.data;if(m.type!=='telemetry')return;info.innerHTML=`<b>${m.alive}</b> life · O₂ ${(m.oxygen*100).toFixed(1)}% · CO₂ ${(m.co2*100).toFixed(2)}% · cave water ${m.caveWater.toFixed(1)} · cloud ${m.cloud.toFixed(1)}`;const s=m.selected;document.getElementById('selected').textContent=s?`Crawler #${s.id} · ${String(s.intent).toUpperCase()} · energy ${Math.round(s.energy*100)}% · O₂ ${Math.round(s.oxygen*100)}% · water ${Math.round(s.hydration*100)}% · gut ${Math.round(s.stomach*100)}% · bladder ${Math.round(s.bladder*100)}%`:'Tap an organism';updateAudio(m);};
function unlockAudio(){if(audio)return;const AC=window.AudioContext||window.webkitAudioContext;if(!AC)return;const ac=new AC(),master=ac.createGain();master.gain.value=0;master.connect(ac.destination);const makeNoise=type=>{const len=ac.sampleRate*2,b=ac.createBuffer(1,len,ac.sampleRate),d=b.getChannelData(0);for(let i=0;i<len;i++)d[i]=Math.random()*2-1;const s=ac.createBufferSource();s.buffer=b;s.loop=true;const f=ac.createBiquadFilter();f.type=type==='rain'?'highpass':'lowpass';f.frequency.value=type==='rain'?2800:240;const g=ac.createGain();g.gain.value=0;s.connect(f).connect(g).connect(master);s.start();return g;};audio={ac,master,rain:makeNoise('rain'),water:makeNoise('water')};applyAudio();}
function applyAudio(){if(!audio)return;const t=audio.ac.currentTime;audio.master.gain.cancelScheduledValues(t);audio.master.gain.linearRampToValueAtTime(sound?.2:0,t+.18);}
function updateAudio(m){if(!audio)return;const t=audio.ac.currentTime;audio.rain.gain.setTargetAtTime(Math.min(.42,m.rain*.45+m.cloud*.015),t,.25);audio.water.gain.setTargetAtTime(Math.min(.34,.035+m.flow*.016+m.caveWater*.0008),t,.35);}
