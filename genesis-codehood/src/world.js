...
function field(seed,cx,cy,salt){let total=0,weight=0;for(let oy=-1;oy<=1;oy++)for(let ox=-1;ox<=1;ox++){const w=ox===0&&oy===0?4:(ox===0||oy===0?2:1);total+=noise(seed,Math.floor(cx/2)+ox,Math.floor(cy/2)+oy,salt)*w;weight+=w;}return total/weight;}
...
function biomeFor(seed,cx,cy,elevation,moisture,heat){
  ...
  if(elevation>.73)return 'HIGHLAND';if(moisture>.67&&elevation<.52)return 'MARSH';if(heat>.62&&moisture<.40)return 'DESERT';if(moisture>.53)return 'FOREST';return 'PLAINS';
}
function siteFor(seed,cx,cy,biome){
  const r=noise(seed,cx,cy,61);if(biome==='RIVER')return r>.76?'BRIDGE':'WILDS';if(biome==='RUINS')return 'RUIN';if(biome==='HIGHLAND'&&r>.67)return 'CAVE';
  if(r<.49)return 'WILDS';if(r<.60)return 'PARK';if(r<.70)return 'HOME';if(r<.80)return 'FARM';if(r<.90)return 'WORK';return 'LAB';
}
...
function applySite(resources,site){
  ...
  if(site==='HOME'){r.shelter+=.68;r.jobs+=.12;r.food+=.08;}else if(site==='FARM'){r.food+=.42;r.soil+=.18;r.jobs+=.32;}else if(site==='WORK'){r.jobs+=.74;r.shelter+=.10;r.ore+=.10;}else if(site==='LAB'){r.jobs+=.82;r.learning=1;r.shelter+=.12;}else if(site==='PARK'){r.food+=.18;r.wood+=.12;}else if(site==='CAVE'){r.shelter+=.34;r.ore+=.35;r.jobs+=.12;}else if(site==='RUIN'){r.shelter+=.20;r.ore+=.22;r.jobs+=.08;}else if(site==='BRIDGE'){r.jobs+=.18;r.shelter+=.04;}
  for(const k of ['food','jobs','shelter','wood','ore','water','soil'])r[k]=clamp(r[k]);return r;
}
function ensureSettlement(chunks){
  ...
  for(let i=0;i<wanted.length;i++){const c=candidates[i];if(!c)break;c.type=c.site=wanted[i];Object.assign(c,applySite(biomeResources(c.biome),c.site));}
}
...
function applyLearnedInfrastructure(chunk){
  ...
  const water=Math.round(chunk.flood*100);
  const alarm=learnedCall('IS_AHEAD',water,38);
  if(alarm===1)chunk.floodAlarm=true;
  if(chunk.floodAlarm&&['WORK','LAB','BRIDGE'].includes(chunk.site)){
...
export function createWorld(seed=0xBADC0DE){
  ...
  for(let cy=0;cy<CHUNK_ROWS;cy++)for(let cx=0;cx<CHUNK_COLS;cx++){
    const id=cy*CHUNK_COLS+cx,elevation=.08+.88*field(seed,cx,cy,1),moisture=field(seed,cx,cy,2),heat=field(seed,cx,cy,3),biome=biomeFor(seed,cx,cy,elevation,moisture,heat),site=siteFor(seed,cx,cy,biome),resources=applySite(biomeResources(biome),site);
    const biomeDanger=biome==='MARSH'?.22:biome==='RUINS'?.28:biome==='DESERT'?.14:biome==='HIGHLAND'?.12:.07,danger=clamp(biomeDanger+noise(seed,cx,cy,5)*.24+(site==='RUIN'?.16:0));
  ...
  ensureSettlement(chunks);return {seed,chunks,sweep:0,cursor:0,processed:0};
}
...
export function updateEnvironment(chunk,{floodLevel=0,rain=0}={}){
  if(!chunk.type)chunk.type=chunk.site=inferLegacyType(chunk);if(!chunk.site)chunk.site=chunk.type;if(!chunk.biome)chunk.biome='PLAINS';chunk.learning=chunk.site==='LAB'?1:0;
  chunk.drainage=(chunk.drainage||0)*.985;
  const naturalWater=chunk.biome==='RIVER'?.22:chunk.biome==='MARSH'?.08:0,floodPressure=Math.max(0,floodLevel-(chunk.elevation*.72));
  chunk.flood=clamp(naturalWater+floodPressure*1.8+rain*.12-chunk.drainage);
  applyLearnedInfrastructure(chunk);
  chunk.flood=clamp(chunk.flood-chunk.pumpRate/180);
  const foodRegen=chunk.site==='FARM'?.045:chunk.biome==='FOREST'?.020:chunk.biome==='MARSH'?.026:chunk.site==='PARK'?.020:.006,jobRegen=chunk.site==='LAB'?.028:chunk.site==='WORK'?.023:chunk.site==='FARM'?.010:chunk.site==='HOME'?.005:.002,dry=1-chunk.flood;
  chunk.food=clamp(chunk.food+foodRegen*dry-chunk.flood*.007);chunk.jobs=clamp(chunk.jobs+jobRegen*dry-chunk.flood*.004);
  if(chunk.site==='HOME'||chunk.site==='CAVE')chunk.shelter=clamp(chunk.shelter+.0015*dry);if(chunk.biome==='FOREST')chunk.wood=clamp((chunk.wood??.4)+.002*dry);if(chunk.biome==='RIVER'||chunk.biome==='MARSH')chunk.water=1;chunk.danger=clamp(Math.max(.02,chunk.danger*.9992+chunk.flood*.0012));
}
