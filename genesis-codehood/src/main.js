...
let frames=0,batchesThisSec=0,sweepsThisSec=0,lastStat=performance.now(),lastDraw=0;
...
function placeFounders(){
  ...
  for(let i=0;i<agents.length;i++){
    const sites=world.chunks.filter(c=>c.site===preferred[i]);
    const c=sites[i%Math.max(1,sites.length)]||world.chunks.find(x=>x.site==='HOME')||world.chunks[0];
    agents[i].x=c.cx*CHUNK_SIZE+CHUNK_SIZE*.5+(i-2)*5;agents[i].y=c.cy*CHUNK_SIZE+CHUNK_SIZE*.5;
    agents[i].tx=agents[i].x;agents[i].ty=agents[i].y;
  }
}
function resetZero(){
  ...
  for(let i=0;i<Math.min(5,popCap);i++)spawn();
  placeFounders();rebuildOccupancy(world,agents);selected=agents[0]||null;
  log('NEW WORLD: five people woke up with zero solved programs.','good');
  log('Follow one person, then open BUILDS when a skill reaches 96/96.','info');
  renderPanel();
}

function currentChunk(a){return world.chunks[chunkIndexFor(a.x,a.y)];}
function localPeer(c,a){
  if(c.agents.length<2)return null;
  for(let i=0;i<6;i++){const b=c.agents[(Math.random()*c.agents.length)|0];if(b&&b!==a&&b.alive)return b;}
  return null;
}
...
function move(a,mult=1){
  ...
  a.x=clamp(a.x+a.vx*5*mult,0,WORLD_W-1);a.y=clamp(a.y+a.vy*5*mult,0,WORLD_H-1);
}
function programWork(a,c){
  ...
  if(out.shipped&&out.improved){
    ...
    artifacts.unshift({name:`${a.task.toLowerCase()}_${a.id}_v${skill.version}.vm`,task:a.task,program:skill.program,author:a.label,tick,verified:true});
    artifacts.length=Math.min(120,artifacts.length);
    log(`★ ${a.name} LEARNED ${a.task}: 96/96 executable tests passed.`,'good');
  }
  if(Object.keys(library).length>before){
    const unlocked=unlockedTaskNames(library);log(`NEW CAPABILITIES: ${unlocked.join(' · ')}`,'money');
    for(const person of agents)retargetAgent(person,library,Math.random);
  }
}
function fight(a,b,c){
  ...
  if(atk>def*1.55&&b.energy<11&&Math.random()<.08){b.alive=false;deaths++;murders++;c.deaths++;log(`A fight near ${c.biome.toLowerCase()} ${c.cx},${c.cy} killed ${b.name}.`,'bad');}
}
function act(a,c,action){
  a.lastAction=action;a.actionTicks++;a.energy-=.16;a.money=Math.max(0,a.money-.008);if(a.money<=0)a.energy-=.05;
  a.energy-=c.flood*(action==='SHELTER'?.10:.58);
  if(action==='FORAGE'){
    const take=Math.min(c.food,.018+.012*Math.random());c.food-=take;a.energy+=take*230;a.stats.food+=take;if(take<.004)a.energy-=.18;
  }else if(action==='WORK'){
    const wage=.08+c.jobs*.48;c.jobs=Math.max(0,c.jobs-.0035);a.money+=wage;totalMoney+=wage;a.energy-=.18;
    if(a.age-a.lastJob>=2){programWork(a,c);a.lastJob=a.age;}
    if(a.role==='CRITIC'||a.role==='TESTER'||a.role==='SECURITY'){
      const t=localPeer(c,a);if(t){const f=critiqueAgent(a,t);if(f&&Math.random()<.04)log(`${a.name} found a failing ${t.task} case: (${f.a}, ${f.b}).`,'bad');}
    }
  }else if(action==='SHELTER'){
    a.energy+=.22*c.shelter;a.stats.shelter++;a.tx=c.cx*CHUNK_SIZE+CHUNK_SIZE*.5;a.ty=c.cy*CHUNK_SIZE+CHUNK_SIZE*.5;
  }else if(action==='EXPLORE'){
    a.energy-=.06;if(Math.hypot(a.tx-a.x,a.ty-a.y)<20||Math.random()<.14){const n=neighboringChunks(world,c);if(n.length)targetChunk(a,n[(Math.random()*n.length)|0]);}
  }else if(action==='REST'){
    a.energy+=.44*c.shelter+.08;a.vx*=.5;a.vy*=.5;
  }else if(action==='SOCIAL'){
    const b=localPeer(c,a);if(b){socialLearn(a,b,Math.random);a.tx=b.x;a.ty=b.y;}a.energy-=.03;
...
function stepAgent(a,c){
  ...
  let action=chooseAction(a.brain,obs(a,c),Math.random);
  const coder=['BUILDER','TESTER','OPTIMIZER','ARCHITECT'].includes(a.role);
  if(coder&&c.jobs>.55&&a.energy>34&&Math.random()<(a.age<45?.42:.22))action='WORK';
  act(a,c,action);
  if(a.energy>78&&a.money>80&&a.age>120&&agents.length<popCap&&Math.random()<.0015){a.energy-=11;a.money-=30;const child=spawn(a);if(child){a.stats.children++;c.births++;}}
  if(a.energy<=0||a.age>9000+Math.random()*3000){a.alive=false;deaths++;c.deaths++;}
}
...
function finishSweep(){
  ...
  if(Math.random()<.006){rain=.55+Math.random()*.45;log('Storm front arrived. River and low marshes are rising.','info');}
  if(!agents.length){paused=true;log('CIVILIZATION EXTINCT — make a NEW WORLD to try another lineage.','bad');}
}
...
function renderPerson(){
  const a=selected&&selected.alive?selected:agents[0];
  if(!a){panel.innerHTML='<section><h2>No living people</h2><p>Start a new world.</p></section>';return;}
  selected=a;const c=currentChunk(a),report=evaluateProgram(a.bestProgram||a.program,a.task,VERIFY_CASES),sig=behaviorSignature(a.brain);
  const roster=agents.map(p=>`<button class="person-chip ${p===a?'selected':''}" data-agent="${p.id}">☺ ${esc(p.name)}<small>${p.role} · ${p.lastAction}</small></button>`).join('');
  panel.innerHTML=`<section class="explain"><h2>Your people</h2><p>Pick one. You only start with five.</p><div class="people-list">${roster}</div></section>
  ...
  <section><h3>What are they trying to learn?</h3><div class="lesson-name">${a.task}</div><p>${TASKS[a.task].label}</p><div class="progress-line">${bar(report.passRate)} <b>${report.passed}/96 tests</b></div><p class="note">Last coding cycle tried ${a.lastAttempts||0} executable variants. Only 96/96 becomes shared knowledge.</p><pre class="code">${esc(disassemble(a.bestProgram||a.program))}</pre></section>
  <section><h3>Survival policy</h3><div class="facts"><span>hungry → <b>${sig.hungry}</b></span><span>flooded → <b>${sig.flooded}</b></span><span>broke → <b>${sig.broke}</b></span><span>danger → <b>${sig.danger}</b></span></div></section>`;
...
function renderLearning(){
  ...
  panel.innerHTML=`<section class="explain"><h2>Programming school</h2><p><b>No solution programs are loaded.</b> They mutate primitive stack/control-flow programs, execute them, keep improvements, and share a skill only after all 96 tests pass.</p></section>`+
  Object.entries(TASKS).map(([name,t])=>{
    ...
    return `<section class="lesson ${learned?'learned':locked?'locked':''}"><div class="row"><b>${name}</b><span>${learned?'✓ VERIFIED':locked?'LOCKED':`${Math.round(rate*96)}/96`}</span></div><div>${t.label}</div><div class="progress-line">${bar(rate)} ${pct(rate)}</div><div class="note">Used by: ${projectUses(name)||'later systems'}</div>${learned?`<pre class="code">${esc(disassemble(learned.program))}</pre>`:''}</section>`;
  }).join('');
}
function renderProjects(){
  const cards=PROJECTS.map(p=>{
    const prog=projectProgress(p,library),ready=projectReady(p,library),installed=ready&&p.kind==='WORLD TOOL';
    return `<section class="project ${ready?'ready':''}"><div class="row"><div><b>${p.title}</b> <span class="kind">${p.kind}</span></div><span>${installed?'INSTALLED':ready?'READY':`${prog.learned.length}/${prog.total}`}</span></div><p>${p.description}</p><div class="progress-line">${bar(prog.ratio)} ${pct(prog.ratio)}</div><div class="note">${ready?(installed?'This verified evolved code now affects the world.':'Every required runtime module is verified.'):`Still needs: ${prog.missing.join(', ')}`}</div><button data-project="${p.id}" ${ready?'':'disabled'}>${ready?'▶ OPEN / TEST':'LOCKED'}</button></section>`;
  }).join('');
  panel.innerHTML=`<section class="explain"><h2>Things they can actually build</h2><p>Training games prove basics. World tools change the settlement. Later builds become automation, navigation and simulations.</p></section>${activeProject?renderProjectRunner():''}${cards}`;
...
function renderProjectRunner(){
  ...
  const manual=p.id==='HAPPY_WALKER'||p.id==='COIN_CHASE';
  const controls=manual?'<button id="projLeft">◀ LEFT</button><button id="projRight">RIGHT ▶</button>':'<button id="projStep">RUN / STEP</button><button id="projRun">RUN ×20</button>';
  return `<section class="runner-box"><div class="row"><b>RUNNING: ${p.title}</b><button id="closeProject">×</button></div><pre class="game-screen">${esc(renderProjectAscii(projectState))}</pre><div class="runner-controls">${controls}</div><div class="note">This runner calls verified evolved bytecode at runtime.</div></section>`;
}
...
function renderWorldPanel(){
  ...
  panel.innerHTML=`<section class="explain"><h2>How to read the map</h2><p>This is geography, not a green matrix. Terrain changes food, water, wood, ore, shelter, danger and flooding.</p><div class="legend">${LEGEND.map(([g,n])=>`<span><b>${g}</b> ${n}</span>`).join('')}</div></section>
  <section><h3>World right now</h3><div class="facts"><span>people <b>${agents.length}</b></span><span>flood alarms <b>${alarms}</b></span><span>evolved pumps <b>${pumps}</b></span><span>deaths <b>${deaths}</b></span></div><pre>${Object.entries(biomeCounts).map(([k,v])=>`${k.padEnd(10)} ${v}`).join('\n')}</pre></section>
...
function renderHud(){
  const learned=Object.keys(library).length,ready=PROJECTS.filter(p=>projectReady(p,library)).length,pumps=world.chunks.filter(c=>c.pumpActive).length;
  hud.innerHTML=`<b>☺ ${agents.length} PEOPLE</b> · day ${day.toFixed(1)} · verified skills ${learned}/${Object.keys(TASKS).length} · builds ${ready}/${PROJECTS.length}<br>${learned?'Civilization knowledge is growing.':'Nobody knows a verified program yet.'}${pumps?` · ${pumps} evolved pump${pumps===1?'':'s'} running`:''}<br><span class="note">Tap a ☺ person. Open BUILDS to see what their code can actually do.</span>`;
}
function draw(){drawAsciiWorld(ctx,world,agents,selected?.id,W,H);renderHud();frames++;}

canvas.addEventListener('pointerdown',e=>{
  const r=canvas.getBoundingClientRect(),tx=clamp(Math.floor((e.clientX-r.left)/r.width*MAP_COLS),0,MAP_COLS-1),ty=clamp(Math.floor((e.clientY-r.top)/r.height*MAP_ROWS),0,MAP_ROWS-1),wp=mapTileToWorld(tx,ty);
  let best=null,bd=(WORLD_W/MAP_COLS*2.5)**2;for(const a of agents){if(!a.alive)continue;const d=(a.x-wp.x)**2+(a.y-wp.y)**2;if(d<bd){bd=d;best=a;}}
...
$('turbo').addEventListener('click',()=>{turbo=turbo===1?4:turbo===4?16:turbo===16?64:1;$('turbo').textContent=`⚡ EVOLVE ×${turbo}`;});
$('rain').addEventListener('click',()=>{rain=1;flood=Math.min(1,flood+.10);log('You triggered a storm. Watch the river, marshes and learned flood tools.','info');});
...
$('save').addEventListener('click',()=>{localStorage.setItem('genesis-codehood-v6',JSON.stringify(serialize()));log('World saved on this device.','good');});
...
function load(){
  try{
    const raw=localStorage.getItem('genesis-codehood-v6');if(!raw)return false;const s=JSON.parse(raw);
    tick=s.tick||0;day=s.day||0;rain=s.rain||0;flood=s.flood||0;totalMoney=s.totalMoney||0;births=s.births||0;deaths=s.deaths||0;murders=s.murders||0;jobs=s.jobs||0;nextId=s.nextId||1;
    const saved=s.library||{};library=emptyLibrary();Object.assign(library,saved);
    artifacts=s.artifacts||[];agents=(s.agents||[]).slice(0,Math.max(5,popCap));world=s.world||createWorld();
    rebuildOccupancy(world,agents);selected=agents[0]||null;for(const a of agents)retargetAgent(a,library,Math.random);
    log('Loaded your Codehood v6 world.','good');return true;
  }catch(err){console.warn(err);return false;}
}
function boot(){
  const steps=[['GENERATING BIOMES',16],['CARVING RIVER + CAVES',32],['PLACING RUINS + SETTLEMENT',50],['SPAWNING FIVE PEOPLE',66],['INDEXING WORLD CHUNKS',78],['STARTING ZERO-SOLUTION SCHOOL',92],['READY',100]];
...
function loop(now){
  if(!paused){const start=performance.now();for(let i=0;i<turbo;i++){simBatch();if(performance.now()-start>11)break;}}
