...
function resetZero(){
  ...
  log('No programming solutions were loaded. Every project module starts unlearned.','info');
  renderPanel();
}
...
function programWork(a,c){
  ...
  if(out.shipped&&out.improved){
    ...
    log(`★ LEARNED ${a.task}: ${a.label} passed 96/96 tests`,'good');
  }
...
function renderPerson(){
  ...
  panel.innerHTML=`
    ...
    <section><h3>What this person is learning</h3><div class="lesson-name">${a.task}</div><p>${TASKS[a.task].label}</p><div class="progress-line">${bar(report.passRate)} <b>${report.passed}/96 tests</b></div><pre class="code">${esc(disassemble(a.bestProgram||a.program))}</pre></section>
...
function renderLearning(){
  ...
  panel.innerHTML=`<section class="explain"><h2>Programming school</h2><p><b>Language 0: CodeVM.</b> They are not handed solutions. A learner mutates executable instruction sequences, runs them, keeps better attempts, and a skill is learned only at 96/96 tests.</p></section>`+
...
function renderProjectRunner(){
  ...
  return `<section class="runner-box"><div class="row"><b>RUNNING: ${p.title}</b><button id="closeProject">×</button></div><pre class="game-screen">${esc(renderProjectAscii(projectState))}</pre><div class="runner-controls">${controls}</div><div class="note">This program calls the civilization's verified genomes at runtime.</div></section>`;
}
...
function renderHud(){
  ...
  hud.innerHTML=`<b>DAY ${day.toFixed(1)}</b> · ☺ ${agents.length.toLocaleString()} · learned ${learned}/${Object.keys(TASKS).length} · programs ${ready}/${PROJECTS.length}<br>world sweep ${bar(world.processed)} ${pct(world.processed)} · flood ${pct(flood)} · speed ×${turbo}`;
}
...
function serialize(){return {version:4,tick,day,rain,flood,totalMoney,births,deaths,murders,jobs,nextId,library,artifacts,agents:agents.slice(0,5000),world};}
$('save').addEventListener('click',()=>{localStorage.setItem('genesis-codehood-v4',JSON.stringify(serialize()));log('World saved on this device.','good');});
...
function load(){
  try{const raw=localStorage.getItem('genesis-codehood-v4');if(!raw)return false;const s=JSON.parse(raw);tick=s.tick||0;day=s.day||0;rain=s.rain||0;flood=s.flood||0;totalMoney=s.totalMoney||0;births=s.births||0;deaths=s.deaths||0;murders=s.murders||0;jobs=s.jobs||0;nextId=s.nextId||1;library=s.library||{};artifacts=s.artifacts||[];agents=s.agents||[];world=s.world||createWorld();rebuildOccupancy(world,agents);log('Loaded saved world.','good');return true;}catch(err){console.warn(err);return false;}
}

function boot(){
  const steps=[['GENERATING WORLD',18],['BUILDING ROADS + HOMES',38],['SPAWNING ZERO-SEED LEARNERS',62],['INDEXING CHUNKS',78],['STARTING PROGRAMMING SCHOOL',92],['READY',100]];
  let i=0;const next=()=>{const [label,n]=steps[i++];$('bootLabel').textContent=label;$('bootPercent').textContent=`${n}%`;$('bootBar').style.width=`${n}%`;$('bootDetail').textContent=i<steps.length?'Nothing solved is being loaded.':'Click a ☺ person or open PROJECTS.';if(i<steps.length)setTimeout(next,70);else setTimeout(()=>$('boot').classList.add('done'),160);};next();
}
