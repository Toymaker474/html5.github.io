...
// Executable programming skills used by real in-app projects.
// The verifier defines behavior, never the implementation. Learners start random.
export const TASKS = Object.freeze({
  STEP_RIGHT: {level:1,fn:(a)=>a+1,reward:120,label:'Game input: move one step right'},
  STEP_LEFT:  {level:1,fn:(a)=>a-1,reward:120,label:'Game input: move one step left'},
  ...
  BOUNCE:     {level:2,fn:(a)=>-a,reward:175,label:'Game physics: reverse a velocity'},
  IS_AHEAD:   {level:2,fn:(a,b)=>a>b?1:0,reward:185,label:'Game AI: is target ahead?'},

  PICK_HIGH:  {level:3,fn:(a,b)=>Math.max(a,b),reward:210,label:'Branching: choose the higher value'},
  PICK_LOW:   {level:3,fn:(a,b)=>Math.min(a,b),reward:210,label:'Branching: choose the lower value'},

  DISTANCE:   {level:4,fn:(a,b)=>Math.abs(a-b),reward:255,label:'Game/sim logic: absolute one-axis distance'},
  MANHATTAN:  {level:5,fn:(a,b)=>Math.abs(a)+Math.abs(b),reward:360,label:'Pathfinding: |dx| + |dy| grid distance'}
});

export function makeCaseSet(count=96,seed=0xC0DE){
  let x=seed>>>0;
  const rng=()=>{x^=x<<13;x^=x>>>17;x^=x<<5;return(x>>>0)/4294967296;};
