...
{
 const d=new DeepWorld({seed:7003,cell:8,cols:32,rows:22,chunkSize:8});d.mat.fill(MAT.AIR);d.water.fill(0);for(let x=0;x<32;x++)d.mat[d.idx(x,18)]=MAT.STONE;for(let x=2;x<30;x++)d.mat[d.idx(x,17)]=MAT.SOIL;d.mat[d.idx(15,17)]=MAT.STONE;d.mat[d.idx(15,16)]=MAT.STONE;d.mat[d.idx(15,15)]=MAT.STONE;d.recomputeSupport();const p=planCellPath(d,{x:d.wx(4),y:d.wy(16)},{x:d.wx(27),y:d.wy(16)},{maxExpanded:5000,profile:{cling:false,swim:false}});assert.equal(p.reachable,true);assert.ok(p.points.length>2);assert.ok(p.points.some(q=>q.y<16),JSON.stringify(p.points));console.log('PASS deep A* routes around solid terrain instead of walking straight through it');
}
{
