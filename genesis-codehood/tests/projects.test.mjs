...
assert.ok(PROJECTS.length>=8,'project ladder should include games, tools and simulations');
...
const sampleAgent={id:1,alive:true,x:600,y:400,lastAction:'EXPLORE'};
const rows=makeAsciiFrame(world,[sampleAgent],1);
assert.equal(rows.length,MAP_ROWS);assert.equal(rows[0].length,MAP_COLS);assert.ok(rows.join('\n').includes('☻'),'selected person should be visible in ASCII world');
console.log(`PASS: ${PROJECTS.length} runnable project cartridges + ${MAP_COLS}x${MAP_ROWS} fortress ASCII renderer.`);
