...
void genesis_body_clear();void genesis_body_set_gravity(int x,int y,int z);int genesis_body_add_node(int x,int y,int z,int mass,int radius);int genesis_body_apply_impulse(int i,int ix,int iy,int iz);int genesis_body_seed_organism(int x,int y,int z);int genesis_body_node_count();int genesis_body_node_y(int i);int genesis_body_com_y();uint32_t genesis_body_contacts();uint32_t genesis_body_step_index();
int genesis_coupled_sync_solids();void genesis_coupled_step(int worldIterations,int bodySteps);uint32_t genesis_coupled_solid_count();uint32_t genesis_coupled_sand_count();uint32_t genesis_coupled_rock_count();uint32_t genesis_coupled_water_count();uint32_t genesis_coupled_hash();int genesis_coupled_material_invariant();int genesis_coupled_cache_width();int genesis_coupled_cache_height();int genesis_coupled_cache_depth();
}
...
int main(){
  ...
  // Real ROCK support enters body collision only through exported material state.
  ...
  // Removing native terrain and re-syncing removes support; same body falls to cache floor boundary.
  ...
  // Native WATER never becomes fake solid support.
  ...
  // Native SAND is physical collision occupancy, not renderer-only color.
  ...
  // Combined material+fluid+body replay must be deterministic.
