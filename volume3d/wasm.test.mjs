...
function fixture(e){
  ...
  e.genesis_step(1);const divBefore=e.genesis_fluid_divergence_before()>>>0,divAfter=e.genesis_fluid_divergence_after()>>>0;assert.ok(divAfter<=divBefore,`projection must not increase divergence: ${divBefore} -> ${divAfter}`);assert.ok((e.genesis_fluid_kinetic_energy()>>>0)>0);
  ...
  e.genesis_init(32,24,32,123456,72);e.genesis_seed_scene();e.genesis_impulse_water(24,7,10,6,420,0,160);for(let i=0;i<80;i++)e.genesis_step(1);assert.equal(e.genesis_hash()>>>0,out.hash,'fixed seed + impulse must replay deterministically');return out;
}
const committedPath=new URL('./solver.v1.wasm.b64',import.meta.url);const committed=Buffer.from(fs.readFileSync(committedPath,'utf8').trim(),'base64');const compiledPath=process.argv[2]||null;let compiledResult=null;
if(compiledPath){const compiled=fs.readFileSync(compiledPath);console.log(JSON.stringify({freshCompiledBytes:compiled.byteLength,freshCompiledSha256:crypto.createHash('sha256').update(compiled).digest('hex')}));console.log('FRESH_COMPILED_B64='+compiled.toString('base64'));compiledResult=fixture(await instantiateBytes(compiled,'FRESH_COMPILED_WASM'));console.log('PASS fresh C++ -> WebAssembly momentum/pressure semantic fixture');}
console.log(JSON.stringify({committedBrowserFile:'solver.v1.wasm.b64',committedBrowserBytes:committed.byteLength,committedBrowserSha256:crypto.createHash('sha256').update(committed).digest('hex')}));const committedResult=fixture(await instantiateBytes(committed,'COMMITTED_BROWSER_WASM'));if(compiledResult)assert.deepEqual(compiledResult,committedResult,'versioned browser WASM and freshly compiled C++ WASM must agree on momentum/pressure fixture');
console.log(JSON.stringify({model:'genesis-volume3d-momentum-pressure-v1',committedWasmBytes:committed.byteLength,committedResult,compiledResult}));console.log('PASS GENESIS native discrete 3D fluid momentum/pressure browser + compiled WebAssembly tests');
