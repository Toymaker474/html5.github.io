...
actor MetalLab {
  ...
  func particleBenchmark(count requested: Int, steps requestedSteps: Int) throws -> [String: Any] {
    ...
    let steps = UInt32(max(1, min(requestedSteps, 2_000)))
    ...
    guard
      ...
      var stepsValue = Optional(steps),
      let stepsBuffer = device.makeBuffer(bytes: &stepsValue, length: MemoryLayout<UInt32>.stride),
    ...
    return [
      ...
      "device": device.name,
      "particles": count,
      "stepsPerParticle": Int(steps),
      "particleSteps": Int64(count) * Int64(steps),
